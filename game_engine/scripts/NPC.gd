...
func _ready():
	...
	if hunger_component and is_instance_valid(hunger_component): # <--- ¡NUEVA LÍNEA!
		hunger_component.starved.connect(Callable(self, "_on_npc_starved"))
	else:
		print(str("-----> ¡ERROR! NPC ID ", npc_id, ": HungerComponent no encontrado o no válido en _ready().")) # <--- ¡NUEVA LÍNEA!
...
func initialize_npc(instance_data: Dictionary):
	...
	self.collision_mask = 1 | 2 | 4 # NPC colisiona con Capa 1 (mundo), Capa 2 (jugador) Y Capa 4 (otros NPCs)
	...
	if hunger_component and is_instance_valid(hunger_component): # <--- ¡NUEVA LÍNEA!
		...
		# Puedes añadir un decay_rate a TipoNPC.valores_rol para configurarlo
		...
		print(str("-----> ¡ERROR! NPC ID ", npc_id, ": HungerComponent no encontrado o no válido.")) # <--- ¡NUEVA LÍNEA!
...
func _on_npc_starved(entity_id: int, entity_type: String): # <--- ¡NUEVO SLOT!
